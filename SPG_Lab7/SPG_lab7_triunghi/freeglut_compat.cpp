// GLFW-based implementation of the freeglut API subset used in SPG_Lab6.
...
static double g_timer_ms           = 0.0;
static double g_timer_target       = -1.0;
static int  g_win_x = 200, g_win_y = 200;
static int  g_win_w = 700, g_win_h = 700;
static bool g_redisplay            = false;
static bool g_swapped              = false; // tracks if display() called glutSwapBuffers
...
void glutMainLoop(void)
{
    ...
    while (!glfwWindowShouldClose(g_window)) {
        if (g_display_func) {
            ...
            g_display_func();
            g_redisplay = false;
            // Only swap if the display function didn't already call glutSwapBuffers.
            // (Single-buffer apps use glFlush and rely on this; double-buffer apps
            //  call glutSwapBuffers themselves and must NOT be swapped again.)
            if (!g_swapped) {
                glfwSwapBuffers(g_window);
            }
        }

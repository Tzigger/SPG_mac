...
// 12 muchii cub (24 pts) + 3 axe WCS (6 pts) = 90 float-uri
float points[] = {
...
void display()
{
    ...
    // ── Cub 1: identitate – rosu ───────────────────────────────────────────
    glUniform3f(colorID, 1.0f, 0.0f, 0.0f);
    glUniformMatrix4fv(matrixID, 1, GL_FALSE,
        glm::value_ptr(projectionMatrix * viewMatrix * modelMatrix));
    glDrawArrays(GL_LINES, 0, 24);

    // ── Cub 2: T(0,-8,0) * S(2,2,2) – verde ───────────────────────────────
    ...
    // ── Cub 3: T(8,0,0) * Rz(pi/4) * S(2,1,2) – albastru ─────────────────
    ...
    // ── Cub 4: T(-8,0,0) * Rz(-pi/4) * S(2,1,2) – magenta ────────────────
    modelMatrix = glm::translate(glm::vec3(-8.0f, 0.0f, 0.0f))
                * glm::rotate(-PI / 4.0f, glm::vec3(0.0f, 0.0f, 1.0f))
                * glm::scale(glm::vec3(2.0f, 1.0f, 2.0f));
...
int main(int argc, char** argv)
{
    ...
    glutCreateWindow("SPG - Lab5 Problema 2");
    init();
